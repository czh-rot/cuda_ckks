...
DeviceVector Context::ModUp(const DeviceVector &in) const {
  ...
  if (num_moduli % alpha__ != 0)
    throw std::logic_error("Size does not match (UP); apply RNS-decompose first.");
  DeviceVector raised;
...
__global__ void hadamardCCMultFused_(size_t degree, size_t log_degree,
                              size_t num_primes, const uint64_t* primes,
                              const uint64_t* barret_ratio,
                              const uint64_t* barret_k, const uint64_t* op1,
                              const uint64_t* op2, const uint64_t* p1,
                              const uint64_t* p2, uint64_t* op3_out,
                              uint64_t* op1_out, uint64_t* op2_out) {
  const int i = blockIdx.x * blockDim.x + threadIdx.x;
  const int prime_idx = i >> log_degree;
  const uint64_t prime = primes[prime_idx];
  uint128_t out_op1 = mult_64_64_128(op1[i], p1[i]);
  uint128_t out_op2 = mult_64_64_128(op2[i], p1[i]);
  uint128_t out_op3 = mult_64_64_128(op1[i], p2[i]);
  uint128_t out_op4 = mult_64_64_128(op2[i], p2[i]);
  
  op1_out[i] = barret_reduction_128_64(out_op1, prime, barret_ratio[prime_idx],
                                   barret_k[prime_idx]);
  op3_out[i] = barret_reduction_128_64(out_op4, prime, barret_ratio[prime_idx],
                                   barret_k[prime_idx]);
  auto t1 = barret_reduction_128_64(out_op2, prime, barret_ratio[prime_idx],
                                   barret_k[prime_idx]);   
  auto t2 = barret_reduction_128_64(out_op3, prime, barret_ratio[prime_idx],
                                   barret_k[prime_idx]);   
  op2_out[i] = t1 + t2;                                                            
}

void Context::CCMult(const Ciphertext &ct, const Ciphertext &ct2,
                    Ciphertext &out, DeviceVector &c) const {
  const auto &op1 = ct.getAxDevice();
  const auto &op2 = ct.getBxDevice();
  const auto &p1 = ct2.getAxDevice();
  const auto &p2 = ct2.getBxDevice();
  ...
  hadamardCCMultFused_<<<degree__ * num_primes / 256, 256>>>(
      degree__, param__.log_degree_, num_primes, primes__.data(), barret_ratio__.data(),
      barret_k__.data(), op1.data(), op2.data(), p1.data(), p2.data(), c.data(), op1_out.data(),
      op2_out.data());
}
...
void Context::CCAdd(const Ciphertext &ct1, const Plaintext &pt,
  ...
  // if (op1_ax.size() != pt.size() || op1_ax.size() != out_ax.size())
...
void Context::Add(const Ciphertext &ct1, const Ciphertext &ct2,
  ...
  if (op1_ax.size() != op2_ax.size() || op1_ax.size() != out_ax.size())
    throw std::logic_error("Size does not match (ADD)");
  int gridDim_ = 2048;
  int blockDim_ = 256;
  add_<<<gridDim_, blockDim_>>>(GetKernelParams(), length, op1_ax.data(),
